...
export default function PersonaCapabilityMapPage() {
  const liveAgentCount = agents.filter((a) => a.status === "live").length;

  const metrics = [
    { value: `${personas.length}`, label: "Personas mapped, from Group CFO to Internal Audit." },
    { value: `${problemStatements.length}`, label: "Problem statements captured across Day, Month and Year." },
    { value: `${themes.length}`, label: "Capability themes, from Procure-to-Pay to Board Reporting." },
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-white">
          ...
          <div className="max-w-7xl mx-auto px-6 py-16 sm:py-20">
            <div className="max-w-2xl">
              ...
              <p className="text-lg text-slate-600 leading-relaxed">
                Pick a day, month or year lens on the function, then hover any node to see
        ...
        <MetricsRow
          metrics={metrics}
          source="Illustrative, informed by common operating-model engagement patterns — not any specific client."
        />
